local Create_CCV = CreateClientConVar
local Get_CV = GetConVar
local hook_A = hook.Add
local cvars_ACC = cvars.AddChangeCallback
local surface_SDC = surface.SetDrawColor
local surface_DR = surface.DrawRect
local surface_STC = surface.SetTextColor
local surface_STP = surface.SetTextPos
local surface_SF = surface.SetFont
local surface_DT = surface.DrawText
...
hook_A("HUDPaint", "PropCounterHUDHook2023", function() pcHUD() end)
...
hook_A("OnScreenSizeChanged", "PropCounterAdjustSize",function()
	...
	timer.Simple( 1, function()  hook_A("HUDPaint", "PropCounterHUDHook2023", function() pcHUD() end) end)
end)
...
cvars_ACC("lpc_enabled", function(convar_name,value_old,value_new)
	...
	if value_new == '1' then timer.Simple( 1, function() hook_A("HUDPaint", "PropCounterHUDHook2023", function() pcHUD() end) end)
